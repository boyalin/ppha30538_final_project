...
author: "Zidan Kong (ZidanKONG), Boya Lin (boyalin), and Sijie Wu (sijiewu12) (All from Secion 1)" 
format: 
#    html: 
#        code-overflow: wrap
    pdf:
        include-in-header: 
            text: |
              \usepackage{fvextra}
              \usepackage[top=1cm, bottom=1cm, left=1.91cm, right=1.91cm]{geometry}
...
header-includes:
  - \usepackage{titling}
  - \preauthor{\begin{center}\footnotesize}  
...
**Data**: We use crashes and people data from Chicago Data Portal in the year 2013-2024, and use crashes data from Open Data DC in the year 1899-2024. Moreover, we extract text data by web-scraping from Chicago and DC news websites respectively. Please download all of our datasets (including both initial, unmodified dataframes we download and the final versions of the dataframes) in this [link](https://drive.google.com/drive/folders/1tsSsifhK4LLAWtzXqsEb91gDD3ShDvNx?usp=sharing).
...
![](graphs/Chart1_comparison_time_series.png){width=600px height=200px fig-align="center"}

**Text Analysis**: News articles of Chicago display lower polarity and subjectivity based on the past 100 news articles.

![](graphs/Chart2_Chicago_text_analysis.png){width=300px height=120px}  ![](graphs/Chart3_DC_text_analysis.png){width=300px height=120px}
...
![](graphs/Chart4_bar_chart_of_damage_level_by_gender.png){width=300px height=200px} ![](graphs/Chart5_primary_type_of_causation.png){width=300px height=200px}
...
![](graphs/Chart6_injuries_by_causation.png){width=300px height=200px} ![](graphs/Chart7_injuries_by_road_surface_condition.png){width=300px height=200px}
...
![](graphs/Chart8_shiny1.png){width=400px height=300px} ![](graphs/Chart9_shiny2.png){width=200px height=300px}
...
Adding crosswalks, bike lanes, motorcycle lanes, and signals at high-crash areas. (4) Prioritize road maintenance, provide real-time traffic updates (with radios) and guidance for hazardous conditions. (5) Enforce stricter age restrictions to prevent underage driving
...
df_drivers = df_merged_after_2019.loc[df_merged_after_2019['PERSON_TYPE'] == 'DRIVER']
...
monthly_counts = df_chicago_dc.groupby(
...
monthly_counts['Per_Capita_Crash'] = monthly_counts['Crash_Count'] / \
    monthly_counts['Population']
...
grouped_seatbelt = df_merged_after_2019.groupby('SAFETY_EQUIPMENT')[
    'INJURIES_TOTAL'].mean().reset_index()
...
grouped_belt_fatal = df_merged_after_2019.groupby('SAFETY_EQUIPMENT')[
    'INJURIES_FATAL'].mean().reset_index()
...
```{python}
#| echo: false
%run shiny-app/basic-app/app.py
```

```{python}
# | echo: false
# function to print shiny app code (from ps6)
...
```{python, echo: false}
# app.py for shiny app
print_file_contents("./shiny-app/basic-app/app.py") 
```
    ...
    if response.status_code == 200:
        ...
        for article in soup.find_all('article', class_='item'):
            ...
            if link:
                ...
                link = urljoin(main_page_url, link)
            ...
            try:
                ...
                if article_response.status_code == 200:
                    ...
                    section = article_soup.find(
                        'section', class_='content__body')
                    if section:
                        file.write("\nFull Article:\n")
        ...
        print(
            f"Failed to fetch the main page. Status code: {response.status_code}")
...
with open(output_file, "w", encoding="utf-8") as file:
    for main_page_url in urls:
        ...
        if response.status_code == 200:
            ...
            car_crash_section = soup.find(
                'section', attrs={'data-slug': 'car-crash'})
...
print(
    f"Articles from all pages under 'Car Crash' section have been successfully saved to '{output_file}'")
...
print(
    f"The sentence with the least polarity is: \"{least_polarity_sentence.strip()}\"")
...
tokens = [token.text.lower() for token in nlp(car_crash_Chicago_text)
...
print(
    f"The most frequent word is '{most_common_word}' and it appears {most_common_count} times.")
...
with open(output_file, "w", encoding="utf-8") as file:
    ...
    for main_page_url in urls:
        ...
        if response.status_code == 200:
            ...
            # Finding all articles with 'article' class
            ...
            for article in articles:
                ...
                if time_tag:
                    publication_time_text = time_tag.get_text(
                ...
                file.write(f"Description: {description}\n")
                file.write(
                    f"Publication Time (Text): {publication_time_text}\n")
                file.write("\nFull Article:\n")
                file.write(content)
                file.write("\n" + "-" * 80 + "\n\n")
            ...
            print(
                f"Failed to fetch the main page {main_page_url}. Status code: {response.status_code}")
    ...
    print(
        f"Extracted {total_articles_extracted} articles. Saved to '{output_file}'.")
...
df_sentence_polarities = pd.DataFrame(accident_sentence_polarities)

chart1 = alt.Chart(df_sentence_polarities).mark_line().encode(
    x = alt.X('n', title = 'Sentence Number'),
    y = alt.Y('crash_polarity', title = "Polarity")
).properties(
